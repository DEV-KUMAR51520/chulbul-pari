...
const TopDecorations = ({ showText = false }) => {
  ...
  return (
    <div className="absolute top-0 left-0 w-full flex flex-col items-center pt-8 z-20 pointer-events-none">
      {/* Lights row */}
      ...
      {showText && (
        <motion.div 
          className="flex flex-nowrap whitespace-nowrap gap-1 sm:gap-1.5 text-lg sm:text-xl font-bold tracking-wider sm:tracking-widest mt-2"
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ delay: 0.5, duration: 0.8 }}
        >
          {['H','A','P','P','Y'].map((letter, i) => (
            <span key={'h'+i} className={`drop-shadow-[0_0_8px_currentColor] 
              ${i%4===0 ? 'text-pink-400' : i%4===1 ? 'text-yellow-300' : i%4===2 ? 'text-cyan-400' : 'text-emerald-400'}`}>
              {letter}
            </span>
          ))}
          <span className="w-1.5 sm:w-2"></span>
          {['B','I','R','T','H','D','A','Y'].map((letter, i) => (
            <span key={'b'+i} className={`drop-shadow-[0_0_8px_currentColor] 
              ${(i+5)%4===0 ? 'text-pink-400' : (i+5)%4===1 ? 'text-yellow-300' : (i+5)%4===2 ? 'text-cyan-400' : 'text-emerald-400'}`}>
              {letter}
            </span>
          ))}
        </motion.div>
